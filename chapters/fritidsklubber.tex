...
Dog vurderes det, at de forekommende administrative opgaver er generelle opgaver, og altså ikke enkelttilfælde.\fxnote{Caspar: Skal der ikke være en del på sætningen hvor vi accepterer vores egen viden evt med tilføjelse af "og derfor godt kan anvendes i denne sammenhæng" eller noget lignende}
...
Denne type frivilligt arbejde er individuel fra klub til klub, hvad end det er undervisning, håndtering af det økonomiske eller noget helt andet. \fxnote{Nikolaj: ``noget helt andet'' talesprog? Tristan: Og selv hvis det skal være der, så vil ``noget helt tredje'' vist være mere korrekt, ikke?}

Dog er der nogle administrative opgaver, som går igen blandt fritidsklubberne.
Det er primært opgaver, som omhandler medlemshåndtering, da klubber har en medlemsbase, som skal administreres. \fxnote{Troels: Forslag til erstatningssætning: Det er primært opgaver som omhandler administration af medlemmer.}
...
Sportsklubber kan også have specielle forhold, som spiller ind\fxnote{Marc: ``Spiller ind'' malplaceret, spiller ind på hvad præcist?: Som har indflydelse på hvilke administrative opgaver klubben har}, grundet de forskellige sportsgrene de udbyder, f.eks. ved en skytteklub gælder der specielle regler for våbenlicenser og våbentransport.
...
Sportshaller \fxnote{Tristan: Er ``sportshaller'' ikke det forkerte ord? Hvad med ``sportscentre'' i stedet? I så fald skal det rettes i hele afsnittet.} lægger lokale til mange forskellige sportsklubber.
...
Haller har ofte også en kiosk eller café, hvor sportsudøvere eller arrangementsgæster kan komme og få noget at spise og drikke. \fxnote{Troels: Er dette relevant for projektet? Tristan: Det tror jeg faktisk ikke. Det spiller i hvert fald ikke ind på nogen måde.}
...
\begin{itemize}
  \item Administration af haludlejning og omklædning \fxnote{Slet hal og skriv: udlejning i stedet?? (Søren)}
...
Det varierer, hvordan ungdomsklubber drives. \fxnote{Troels: Det varierer fra klub til klub hvordan den drives. Tristan: Har vi ikke benyttet ``fra klub til klub'' rimeligt mange gange allerede? Ved ikke om variation vil være positivt, da det jo ér en teknisk rapport.}
...
\end{itemize}
\fxnote{Hvilken relevans har Ungdomsklubber i henhold til projektet?? - De har ikke rigtig noget til fælles med sejlklubber, andet end begivenheder. - Det var ment som en tilføjelse til fritidsklubber, da det måske kan virke som om vi har set lidt snævert på fritidsklubber (Thomas)}


\section{Sejlklub}

En sejlklub kan, alt efter størrelsen, have flere administrative opgaver. \fxnote{Tristan: Gælder det ikke generelt, at antallet af administrative opgaver hænger delvist sammen med klubbens størrelse?}
...
\begin{itemize}
  ...
  \item Reservering og udlån af både \fxnote{Troels: Tilføj se hvem og hvornår der reservers både? Se og skriv logbøger (vi nævner dette i det initierende problem.)}
