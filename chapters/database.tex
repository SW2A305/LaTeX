\chapter{Database}\label{chap:database}\fxnote{Marc: afsnittet er generalt lidt lavt på kilder, smid eventuelt en enkelt kilde til EF, SQlite og DAL for at understøtte vores teorier om hvad det er}

Dette kapitel omhandler forsøget på at opnå persistens mellem kørsler af programmet.
Det ønskes naturligvis\fxnote{Marc: at drage sådanne konklusioner bør ikke gøres i en rapport(naturligvis, selvfølgelig, som altid osv.), hvorfra ved man at dette altid er ønsket?}, at programmet kan lagre sit data, udenfor RAM, for at kunne bruge det igen senere. 
...
Efter research, herunder rådgivning fra OOP lektor Rene Hansen, stod valget mellem to databaser: Entity Framework og SQLite.\fxnote{ER vi sikker på han er lektor ? :) (søren)}

\subsubsection*{Entity Framework}
Den første af de to, \ac{EF}, er en Object-Relational Mapping (ORM) til dotNet Frameworket.\fxnote{Nikolaj: Bør der ikke stå: ``.Net''? Thomas: Der står .NET under WPF}
...
Dette gør \ac{EF} til en attraktiv løsning for grupper\fxnote{Troels: Kun grupper ikke individer? Caspar: måske bare programmører}, der gerne vil hurtigt i gang med at programmere en løsning, uden først at sætte sig ind i databaseprogrammering.
...
Målet med SQLite er at bibeholde letvægten, da systemet også benyttes på mange mobile enheder, såsom Android.\fxnote{Marc: Smid eventuelt en kilde på det her? Tristan: Kan ikke finde egentlige tal på det, men SQLite er indbygget i Android API'et. Ved ikke om vi skal bruge dét? http://developer.android.com/guide/topics/data/data-storage.html }
...
Man udstiller et interface til programmet, som så anvendes, og ikke en direkte forbindelse. \fxnote{Søren: Og derfor ikke en? }
...
Efter anbefaling fra OOP lektor Rene Hansen, valgte gruppen at fokusere på \acl{EF} med Code-First, grundet manglende kendskab til \ac{SQL}.\fxnote{Nikolaj: Det er blevet nævnt en gang, bør det slettes her?} 
...
Det ville være en mulighed at gemme data hver gang der trykkes på ``Log ud''--knappen, eller måske at have en decideret ``Gem''--knap. \fxnote{Kom ind på det i konklusionen.}
